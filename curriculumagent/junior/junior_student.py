...
"""
Hugo Cardante -> Commented this out because I don't need it for the project.
If we need it in the future, uncomment, and get nni.
"""
# import nni
...
class Junior:
    ...
    def _extract_config(
        self, config
    ) -> Tuple[List, tf.keras.initializers.Initializer, tf.keras.layers.Layer]:
        """
        ...
        if not any(
            ...
            warnings.warn(
                "The custom dictionary had not the correct keys. Revert to default model."
            )
        ...
        # Initializer Options
        initializer_option = {
            "O": tf.keras.initializers.Orthogonal(),
    ...
    def train(
        ...
        if isinstance(ckpt_dir, (Path, str)):
            ...
            cp_callback = ModelCheckpoint(
                filepath=ckpt_path, save_weights_only=False, save_freq=10, verbose=1
            )
    ...
    def test(
        self, x: np.ndarray, y: np.ndarray, save_path: Optional[Path] = None
    ) -> dict:
...
def train(
    ...
    s_train, a_train, s_validate, a_validate, _, a_test = load_dataset(
    ...
    if action_space_file is None:
        max_action_value = (
            np.max([np.max(a_train), np.max(a_validate), np.max(a_test)]) + 1
        )
    else:
        if isinstance(action_space_file, Path):
            assert action_space_file.is_file()
    ...
    logging.info(
        f"A total of {max_action_value} actions are assumed, based on the action_space_file input."
    )
...
def validate(
    ...
    s_train, a_train, s_validate, a_validate, s_test, a_test = load_dataset(
        dataset_path, dataset_name
    )
    ...
    if action_space_file is None:
        ...
        if isinstance(action_space_file, Path):
            ...
            actions = np.concatenate(
                [np.load(str(act_path)) for act_path in action_space_file], axis=0
            )
            max_action_value = len(actions)

    logging.info(
        f"A total of {max_action_value} are assumed, based on the action_space_file input."
    )

...
import config
from conformalized_models import calculate_difficulty_estimate, extract_test_features
from utils.data_structures import Trajectory


class CPwithKNNPredictor:
    """
    ...
    def __init__(self, alpha: float, k: int, n_lines: int) -> None:
        """
        ...
        self.normalized_quantile: npt.NDArray[np.float64] = None
    ...
    def fit(
        ...
        """
        Trains the model on the calibration data
        ...
        self.normalized_quantile = self._compute_quantiles(features_scaled)
    ...
    def _compute_quantiles(
        ...
        Returns:
            Normalized quantile values per line
        """
        ...
        normalized_quantile = np.quantile(normalized_scores_all, 1 - self.alpha, axis=0)
    ...
    def predict(
        ...
        for ts in trajectory.timestamps:
            ...
            # Scale normalized quantile by difficulty estimate
            thresholds = difficulty_estimates * self.normalized_quantile
...
def compute_model(
    ...
    alpha: float,
    k: int,
) -> CPwithKNNPredictor | None:
    """
    Creates and trains a conformal predictor using k-NN for uncertainty scalars
    ...
    print(f"Creating Conformal Predictor using k-NN (k = {k})")

    knn_predictor = CPwithKNNPredictor(alpha=alpha, k=k, n_lines=n_lines)

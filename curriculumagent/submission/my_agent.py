...
# BUG: ALTEREI ISTO PORQUE NÃO CORRIA COM A VERSÃO QUE TINHA
...
class MyAgent(BaseAgent):
    ...
    def act_with_id(
        ...
        if self.next_actions is not None:
            ...
            try:
                ...
                next_action = find_best_line_to_reconnect(
                    obs=observation, original_action=next_action
                )
        ...
        if observation.rho.max() < self.best_action_threshold:  # fixed threshold
            if self.topo:
                ...
                default_action = find_best_line_to_reconnect(
                    obs=observation, original_action=default_action
                )
            else:
        ...
        for k, idx in enumerate(sorted_actions):
            ...
            if rho_max <= self.best_action_threshold:
                ...
                logging.info(
                    f"take action {idx}, max-rho to {rho_max},"
        ...
        if idx_chosen:
            self.next_actions = split_action_and_return(
                observation, self.action_space, self.actions[idx_chosen, :]
            )
            next_action = next(self.next_actions)
            next_action = find_best_line_to_reconnect(
                obs=observation, original_action=next_action
            ...
            next_action = find_best_line_to_reconnect(
                obs=observation, original_action=self.action_space({})
            )
            idx_chosen = -1
    ...
    def __collect_action(
        self, this_directory_path: str, action_space_path: Union[Path, List[Path]]
    ) -> np.ndarray:
        ...
        if isinstance(action_space_path, Path):
            if action_space_path.is_file():
                logging.info(
                    f"action_space_path {action_space_path} is a file and will be loaded."
                )
                ...
                all_action_files = [
                    act
                    for act in os.listdir(action_space_path)
            ...
            logging.info(
                f"action_space_path {action_space_path} is a list containing multiple actions."
            )
    ...
    def __get_keras_actions_model(self, obs: grid2op.Observation.BaseObservation):
        ...
        if isinstance(self.model, tf.keras.models.Sequential):
            ...
            action_probability = (
                tf.nn.softmax(action_probability_pre_softmax).numpy().reshape(-1)
            )
